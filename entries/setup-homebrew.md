...
Homebrew is the most popular package manager for macOS (and Linux). It lets you install command-line tools, languages,
databases, and other software with a single terminal command — no manual downloads or installers required.
...
After installation, Homebrew lives in `/opt/homebrew` instead of `/usr/local`. The installer will print two commands to
add Homebrew to your shell `PATH` — make sure you run them:
...
If you're not able to install applications into `/Applications` (e.g. you don't have Administrator permissions on a
regular basis) then set the following env var in your profile to direct Homebrew to install casks to `~/Applications`
instead:
...
A `Brewfile` lets you declare all your packages in a single file and install them in one shot — great for reproducible
setups.
